...
#TODO: add universities by counties.
#----------------------universities----------------------------------
#-------------------Cases by county (over time):---------------------
ourURL = "https://raw.githubusercontent.com/nytimes/covid-19-data/master/us-counties.csv"
dataAsText = extract.grabData(ourURL)
wantedCols = ['date', 'fips', 'cases', 'deaths']
df = transform.csvTodf(dataAsText, wantedCols)
#-----------------stay at home---------------------------------------
ourURL = "https://data.cdc.gov/resource/y2iy-8irm.csv"
dataAsText = extract.grabData(ourURL)
#TODO: check if empty and clean each file.

#TODO: Grab county and state from this file.
#only grab the wanted columns and load those into a df.
wantedCols = ['FIPS_State','FIPS_County', 'date', 'Order_code', 'Stay_at_Home_Recommendation']
df = transform.csvTodf(dataAsText, wantedCols)
#-----------------restaurants----------------------------------------
